...
export default function PayrollPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setAdjustments(Object.fromEntries((workspace.run?.results ?? []).filter((item: PayrollResult) => item.payrollType === "fixed" && item.fixedAdjustment).map((item: PayrollResult) => [item.employeeId, item.fixedAdjustment])));
  ...
  const calculate = async () => {
    ...
    try {
      ...
      setPayrollFile(null);
      setTransferFile(null);
      setMessage("給与の試算が完了しました。内容を確認してください。");
    ...
    <main style={s.page}>
      <div style={s.shell}>
        <header style={s.headerRow}>
          ...
          <button type="button" onClick={() => setSettingsOpen(true)} style={s.settingsButton}>正社員給与設定・当月調整</button>
        </header>
          ...
          {run && (
            <>
              ...
              <div style={s.issueSummary}>
                <b>エラー件数：{errors.length}件</b>
              </div>
              {errors.length > 0 && <div style={s.issueBox}>{errors.map((e, i) => <p key={i} style={s.errText}>エラー：{e.message}</p>)}</div>}
              {fixedWarningEmployeeIds.length > 0 && <button type="button" onClick={() => { setSettingsTab("master"); setSettingsOpen(true); }} style={s.warningButton}>正社員{fixedWarningEmployeeIds.length}名に未設定項目があります</button>}
        ...
        <section style={s.card}>
          ...
          {confirmedRuns.length === 0 ? (
            ...
            <>
              ...
              {slipRun && <div style={s.payslipActions}>
        ...
        {settingsOpen && <div style={s.overlay} role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setSettingsOpen(false); }}>
          <section style={s.modal} role="dialog" aria-modal="true" aria-label="正社員給与設定・当月調整">
            <div style={s.modalHeader}><h2 style={s.h2}>正社員給与設定・当月調整</h2><button type="button" onClick={() => setSettingsOpen(false)} style={s.closeButton}>閉じる</button></div>
            <label style={s.lbl}>正社員を選択<select value={settingsTab === "master" ? masterEmployeeId : adjustEmployeeId} onChange={(e) => { setMasterEmployeeId(e.target.value); setAdjustEmployeeId(e.target.value); }} style={s.sel}><option value="">選択してください</option>{fixedEmployeeCandidates.map(([code, name]) => { const employee = fixedMasters.find((item) => item.employeeCode === code); return <option key={code} value={employee?.employeeId ?? `unregistered:${code}`} disabled={!employee}>{code} {name}{employee ? "" : "（未登録）"}</option>; })}</select></label>
            <div style={s.tabs}><button type="button" onClick={() => setSettingsTab("master")} style={settingsTab === "master" ? s.activeTab : s.tab}>固定給与マスター</button><button type="button" onClick={() => setSettingsTab("adjustment")} style={settingsTab === "adjustment" ? s.activeTab : s.tab}>当月調整</button></div>
            {settingsTab === "master" && <>
              {masterForms[masterEmployeeId] ? <div style={s.masterGrid}>{([['fixedBaseSalary','基本給'],['directorCompensation','役員報酬'],['positionAllowance','職能手当'],['businessAllowance','業務手当'],['holidayAllowance','休日手当'],['fixedOvertimeAllowance','固定手当'],['healthInsurance','健康保険'],['childSupportContribution','子育て支援金'],['careInsurance','介護保険'],['employeePension','厚生年金'],['employmentInsurance','雇用保険'],['incomeTax','所得税'],['residentTax','住民税']] as const).map(([key,label]) => <label key={key} style={s.lbl}>{label}<input type="number" min="0" placeholder="未設定" value={masterForms[masterEmployeeId][key] ?? ""} onChange={(e) => setMasterForms((current) => ({ ...current, [masterEmployeeId]: { ...current[masterEmployeeId], [key]: e.target.value === "" ? null : Number(e.target.value) } }))} style={s.input} /></label>)}<label style={s.lbl}>交通費区分<select value={masterForms[masterEmployeeId].transportationType ?? ""} onChange={(e) => setMasterForms((current) => ({ ...current, [masterEmployeeId]: { ...current[masterEmployeeId], transportationType: e.target.value as FixedMaster['transportationType'] } }))} style={s.input}><option value="" disabled>未設定</option><option value="none">なし</option><option value="daily">日額</option><option value="monthly">月額</option></select></label></div> : <p style={s.muted}>正社員を選択してください。</p>}
            ...
            {settingsTab === "adjustment" && <>
              <p style={s.sub}>対象月：{fmtMonth(targetMonth)}</p>
              {(() => { const result = run?.results.find((item) => item.employeeId === adjustEmployeeId && item.payrollType === "fixed"); if (!result) return <p style={s.hint}>先に給与を試算すると当月調整を入力できます。</p>; const value = adjustments[result.employeeId] ?? result.fixedAdjustment ?? {}; const automatic = Math.floor((Number(value.overtimeMinutes) || 0) * (result.earnings.baseSalary / 160) * 1.25 / 60); return <><div style={s.adjustGrid}><label style={s.lbl}>臨時出勤回数<input type="number" min="0" value={value.temporaryAttendanceCount ?? ""} onChange={(e) => updateAdjustment(result.employeeId,{temporaryAttendanceCount:Number(e.target.value)})} style={s.input}/></label><label style={s.lbl}>残業時間（時間）<input type="number" min="0" step="0.25" value={value.overtimeMinutes == null ? "" : value.overtimeMinutes/60} onChange={(e) => updateAdjustment(result.employeeId,{overtimeMinutes:Number(e.target.value)*60})} style={s.input}/></label><label style={s.lbl}>自動計算した残業手当<input readOnly value={automatic} style={s.input}/></label><label style={s.lbl}>残業手当修正額<input type="number" min="0" value={value.overtimeAllowanceOverride ?? ""} onChange={(e) => updateAdjustment(result.employeeId,{overtimeAllowanceOverride:e.target.value===""?null:Number(e.target.value)})} style={s.input}/></label><label style={s.lbl}>修正理由<input value={value.overtimeReason ?? ""} onChange={(e) => updateAdjustment(result.employeeId,{overtimeReason:e.target.value})} style={s.input}/></label>{([['transportation','交通費'],['advanceExpense','立替経費'],['otherTemporaryPayment','その他臨時支給'],['otherDeduction','その他控除']] as const).map(([key,label])=><label key={key} style={s.lbl}>{label}<input type="number" min="0" value={value[key] ?? ""} onChange={(e)=>updateAdjustment(result.employeeId,{[key]:Number(e.target.value)})} style={s.input}/></label>)}<label style={s.lbl}>備考<input value={value.note ?? ""} onChange={(e)=>updateAdjustment(result.employeeId,{note:e.target.value})} style={s.input}/></label></div><div style={s.adjustResult}><span>残業手当 <b>{yen(result.earnings.overtimePremium)}</b></span><span>支給合計 <b>{yen(result.earnings.grossTotal)}</b></span><span>控除合計 <b>{yen(result.deductions.total)}</b></span><span>差引支給額 <b>{yen(result.netPay)}</b></span></div><button disabled={busy} onClick={applyAdjustments} style={s.secondary}>当月調整を反映して再試算</button></>; })()}
            </>}
          </section>
        </div>}
      </div>
